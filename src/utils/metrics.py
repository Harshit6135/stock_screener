...
def calculate_sharpe_ratio(
    returns: pd.Series, risk_free_rate: float = 0.06, periods_per_year: int = 52
) -> float:
    """
...
def calculate_all_metrics(
    ...
    if equity_curve is not None and not equity_curve.empty:
        ...
        metrics["calmar_ratio"] = round(
            calculate_calmar_ratio(metrics["cagr"] / 100, metrics["max_drawdown"] / 100), 2
        )
        metrics["total_return"] = round((final_value - initial_value) / initial_value * 100, 2)
    ...
    if trades:
        ...
        metrics["avg_holding_period_days"] = round(calculate_avg_holding_period(trades), 1)
