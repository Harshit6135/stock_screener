...
class RankingRepository:
    ...
    def delete_after_date(date):
        ...
        try:
            num_deleted = RankingModel.query.filter(RankingModel.ranking_date > date).delete()
            db.session.commit()
            return num_deleted
    ...
    def get_by_symbol(symbol, ranking_date=None):
        ...
        if ranking_date:
            return RankingModel.query.filter(
                RankingModel.tradingsymbol == symbol, RankingModel.ranking_date == ranking_date
            ).first()
        return (
            RankingModel.query.filter(RankingModel.tradingsymbol == symbol)
            .order_by(RankingModel.ranking_date.desc())
            .first()
        )
    ...
    def get_rankings_after_date(after_date):
        ...
        return (
            RankingModel.query.filter(RankingModel.ranking_date > after_date)
