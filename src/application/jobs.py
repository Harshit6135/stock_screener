...
class JobStore:
    ...
    def submit(
        ...
        *,
        ...
        with self._connect() as connection:
            ...
            row = connection.execute("SELECT * FROM ops_jobs WHERE fingerprint = ?", (fingerprint,)).fetchone()
            if row:
                if row["kind"] != kind or row["payload_checksum"] not in {"", checksum}:
                    raise DomainValidationError("idempotency fingerprint was reused with a different command")
            ...
            self._append(connection, job.job_id, "submitted", {"fingerprint": fingerprint, "kind": kind, "payload_checksum": checksum})
            return job
    ...
    def claim_next(self, worker_id: str, lease_seconds: int = 60) -> Job | None:
        ...
        with self._connect() as connection:
            ...
            claimed = connection.execute("SELECT * FROM ops_jobs WHERE job_id = ?", (row["job_id"],)).fetchone()
    ...
    def transition(self, job_id: int, target: JobStatus) -> Job:
        with self._connect() as connection:
            ...
            if target not in _ALLOWED_TRANSITIONS[current]:
                raise DomainValidationError(f"invalid job transition: {current.value} -> {target.value}")
            connection.execute(
                "UPDATE ops_jobs SET status = ?, lease_until = NULL, lease_owner = NULL, claim_token = NULL, updated_at = ? WHERE job_id = ?",
                (target.value, self._now(), job_id),
            )
            self._append(connection, job_id, "status", {"from": current.value, "to": target.value})
    ...
    def complete(self, job_id: int, result: dict[str, Any], claim_token: str | None = None) -> Job:
        ...
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute("SELECT status, claim_token, cancel_requested FROM ops_jobs WHERE job_id = ?", (job_id,)).fetchone()
            ...
            connection.execute(
                ...
                (JobStatus.SUCCEEDED.value, json.dumps(result, sort_keys=True), self._now(), job_id),
            )
    ...
    def fail(
        ...
        *,
        ...
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute("SELECT status, claim_token, cancel_requested, attempts, max_attempts FROM ops_jobs WHERE job_id = ?", (job_id,)).fetchone()
            if row is None or JobStatus(row["status"]) != JobStatus.RUNNING:
            ...
            next_attempt_at = (
                datetime.now(UTC) + timedelta(seconds=min(2 ** row["attempts"], 60))
            ...
            self._append(connection, job_id, "retry_scheduled" if will_retry else "failed", {"error": error})
        return self.get(job_id)

    def request_cancel(self, job_id: int) -> Job:
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute("SELECT status FROM ops_jobs WHERE job_id = ?", (job_id,)).fetchone()
            if row is None:
                raise DomainValidationError("job does not exist")
            if JobStatus(row["status"]) == JobStatus.QUEUED:
                connection.execute("UPDATE ops_jobs SET status = ?, cancel_requested = 1, updated_at = ? WHERE job_id = ?", (JobStatus.CANCELLED.value, self._now(), job_id))
            elif JobStatus(row["status"]) == JobStatus.RUNNING:
                connection.execute("UPDATE ops_jobs SET cancel_requested = 1, updated_at = ? WHERE job_id = ?", (self._now(), job_id))
            else:
    ...
    def heartbeat(self, job_id: int, claim_token: str, lease_seconds: int = 60) -> Job:
        ...
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute(
                "SELECT status, claim_token, cancel_requested FROM ops_jobs WHERE job_id = ?",
                (job_id,),
            ).fetchone()
            if row is None or JobStatus(row["status"]) != JobStatus.RUNNING:
    ...
    def emit(self, job_id: int, event_type: str, payload: dict[str, Any]) -> None:
        ...
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            if not connection.execute("SELECT 1 FROM ops_jobs WHERE job_id = ?", (job_id,)).fetchone():
                raise DomainValidationError("job does not exist")
            self._append(connection, job_id, event_type, payload)

    def events_after(self, job_id: int, event_id: int = 0, limit: int = 500) -> list[dict[str, Any]]:
        if event_id < 0 or not 1 <= limit <= 500:
            raise DomainValidationError("event cursor or limit is invalid")
        with self._connect() as connection:
            rows = connection.execute("SELECT event_id, event_type, payload_json, created_at FROM ops_job_events WHERE job_id = ? AND event_id > ? ORDER BY event_id LIMIT ?", (job_id, event_id, limit)).fetchall()
    ...
    def _append(self, connection: sqlite3.Connection, job_id: int, event_type: str, payload: dict[str, Any]) -> None:
        connection.execute("INSERT INTO ops_job_events(job_id, event_type, payload_json, created_at) VALUES (?, ?, ?, ?)", (job_id, event_type, json.dumps(payload, sort_keys=True), self._now()))
