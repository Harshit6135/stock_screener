...
from schemas import MarketDataQuerySchema, MarketDataSchema, MaxDateSchema
from services import MarketDataService

blp = Blueprint(
    "Market Data",
    __name__,
    url_prefix="/api/v1/marketdata",
    description="Operations on market data",
)
...
class MarketDataUpdateAllHistorical(MethodView):
    ...
    def post(self):
        ...
        marketdata_service = MarketDataService()
        marketdata_service.update_latest_data_for_all(
            historical=True, historical_start_date=BACKTESTING_HISTORY_START_DATE
        )
...
class MarketDataBySymbol(MethodView):
    ...
    def get(self, tradingsymbol: str):
        ...
        result = marketdata_repo.get_marketdata_by_trading_symbol(tradingsymbol, as_of_date)
