...
from utils import calculate_capital_gains_tax, calculate_tax_adjusted_cost, should_hold_for_ltcg

blp = Blueprint(
    "Tax Analysis", __name__, url_prefix="/api/v1/tax", description="Tax Calculation Operations"
)
...
class TaxEstimate(MethodView):
    ...
    def get(self):
        ...
        if not all(
            ...
            abort(
                400,
                message="Missing required parameters: purchase_price, "
...
class HoldForLTCG(MethodView):
    ...
    def get(self):
        ...
        if not all([purchase_date_str, current_date_str, current_score_str]):
            abort(
                400,
                message="Missing required parameters: purchase_date, "
        ...
        result = should_hold_for_ltcg(
            purchase_date=purchase_date, current_date=current_date, current_score=current_score
        )
