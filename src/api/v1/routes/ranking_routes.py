...
from repositories import MarketDataRepository, RankingRepository
from schemas import MessageSchema, RankingSchema, TopNSchema
from services import RankingService
from utils import get_prev_friday

blp = Blueprint(
    "Rankings", __name__, url_prefix="/api/v1/ranking", description="Operations on Weekly Rankings"
)
    ...
    def get(self, symbol):
        ...
        if actual_date:
            price_data = marketdata_repo.query(
                {"tradingsymbol": symbol, "start_date": actual_date, "end_date": actual_date}
            )
