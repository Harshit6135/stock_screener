def score_rsi_regime(self, rsi: pd.Series) -> pd.Series:
        """
    ...
    def score_trend_extension(self, dist_200: pd.Series) -> pd.Series:
        """
    ...
    def score_percent_b(self, percent_b: pd.Series) -> pd.Series:
        """
...
def rsi_regime_score(rsi_series):
    """Non-linear RSI scoring for momentum regime"""
    def score_rsi(rsi):
        if pd.isna(rsi):
            return 0
        elif rsi < 40:
    ...
    return rsi_series.apply(score_rsi)
