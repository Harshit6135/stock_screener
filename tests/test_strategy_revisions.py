...
def test_strategy_revisions_and_snapshots_are_versioned_by_input_ids():
    ...
    revision_a = StrategyRevision("momentum", uuid4(), "1.0.0", policy.revision_id, {"trend": Decimal("1")}, (configuration.configuration_id,))
    revision_b = StrategyRevision("momentum", uuid4(), "1.1.0", policy.revision_id, {"trend": Decimal("0.5"), "momentum": Decimal("0.5")}, (configuration.configuration_id,))
    members = (RankingMember("ABC", Decimal("90"), True, "eligible"), RankingMember("XYZ", Decimal("80"), True, "eligible"))

    snapshot_a = RankingSnapshot.create(date(2026, 1, 2), revision_a.revision_id, uuid4(), uuid4(), members)
    snapshot_b = RankingSnapshot.create(date(2026, 1, 2), revision_b.revision_id, uuid4(), uuid4(), members)
