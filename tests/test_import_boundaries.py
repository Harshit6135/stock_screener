...
DOMAIN_PACKAGES = {"platform_kernel", "portfolio_engine", "portfolio_accounting", "reference_data", "market_data", "indicators", "strategies", "backtesting"}
FORBIDDEN_ROOTS = {"flask", "sqlalchemy", "kiteconnect", "yfinance", "db", "models", "repositories", "services"}
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    ...
                    if node.module.startswith("src.") and node.module.count(".") > 1:
                        violations.append(f"{file.relative_to(source_root)} imports internal module {node.module}")
                else:
                    continue
                for root in roots:
