...
OUTPUT_DIR = "data"
WEEKLY_SCORES_FILE = "data/exports/weekly_composite_scores.csv"
...
MAX_POSITIONS = 15             # Maximum concurrent positions (soft limit)
...
TOP_N = 100                               # Target number of stocks in portfolio/consideration
TOP_N_CANDIDATES = 200                    # Candidates to fetch before filtering
BUFFER_PERCENT = 0.25          # Swap buffer - challenger must beat incumbent by this % (25% = 0.25)

# New Filters
MIN_PRICE_THRESHOLD = 75.0               # Exclude stocks below this price
MIN_MARKET_CAP_CRORE = 500.0             # Exclude stocks below this market cap (in Crores)
YFINANCE_DUMP_FILE = "data/exports/yfinance_dump.csv"
...
class WeeklyBacktester:
    ...
    def __init__(self, start_date: date, end_date: date, initial_capital: float = INITIAL_CAPITAL):
        ...
        if os.path.exists(WEEKLY_SCORES_FILE):
            self.scores_df = pd.read_csv(WEEKLY_SCORES_FILE)
            self.scores_df['date'] = pd.to_datetime(self.scores_df['date']).dt.date
        else:
            print(f"Warning: {WEEKLY_SCORES_FILE} not found!")
            self.scores_df = pd.DataFrame()

        # Load shares outstanding for market cap filter
        self.shares_map = {}
        if os.path.exists(YFINANCE_DUMP_FILE):
            try:
                dump_df = pd.read_csv(YFINANCE_DUMP_FILE)
                # Create map for both NSE and BSE symbols
                for _, row in dump_df.iterrows():
                    shares = row.get('sharesOutstanding')
                    if pd.notna(shares):
                        if pd.notna(row.get('NSE_SYMBOL')):
                            self.shares_map[str(row['NSE_SYMBOL'])] = float(shares)
                        if pd.notna(row.get('BSE_SYMBOL')):
                            self.shares_map[str(row['BSE_SYMBOL'])] = float(shares)
                print(f"Loaded shares outstanding data for {len(self.shares_map)} symbols")
            except Exception as e:
                print(f"Error loading {YFINANCE_DUMP_FILE}: {e}")
        else:
            print(f"Warning: {YFINANCE_DUMP_FILE} not found! Market cap filter will be disabled.")
    ...
    def fetch_rankings(self, ranking_date: date) -> List[dict]:
        """Fetch top candidates from CSV for a given date"""
        if self.scores_df.empty:
            return []
        
        day_scores = self.scores_df[self.scores_df['date'] == ranking_date]
        if day_scores.empty:
            return []
            
        # Already sorted in avg_score.py, but just in case
        day_scores = day_scores.sort_values('composite_score', ascending=False)
        return day_scores.head(TOP_N_CANDIDATES).to_dict('records')
    
    def fetch_ranking_by_symbol(self, symbol: str, ranking_date: date) -> Optional[dict]:
        """Fetch ranking for a specific symbol from CSV on a given date"""
        if self.scores_df.empty:
            return None
            
        match = self.scores_df[(self.scores_df['date'] == ranking_date) & (self.scores_df['tradingsymbol'] == symbol)]
        if not match.empty:
            return match.iloc[0].to_dict()
        return None
    
    def fetch_price_with_fallback(self, symbol: str, target_date: date) -> Optional[dict]:
        """Fetch OPEN price for a symbol on target_date, with fallback to next 4 days"""
        for i in range(5):
            current_try = target_date + timedelta(days=i)
            try:
                # Specific GET as requested
                response = requests.get(
                    f"{BASE_URL}/api/v1/marketdata/query",
                    json={
                        "tradingsymbol": symbol,
                        "start_date": current_try.strftime("%Y-%m-%d"),
                        "end_date": current_try.strftime("%Y-%m-%d")
                    },
                    timeout=5
                )
                if response.status_code == 200:
                    ...
                    if data and len(data) > 0:
                        # Use OPEN price as requested
                        return {
                            "price": data[0].get('open'),
                            "date": current_try
                        }
            except Exception:
                continue
    ...
    def rebalance_portfolio(self, week_date: date, top_rankings: List[dict], 
        ...
        for symbol, pos in list(self.positions.items()):
            ...
            # Use fallback for challenger price (Open price)
            challenger_data = self.fetch_price_with_fallback(challenger_symbol, week_date)
            challenger_price = challenger_data['price'] if challenger_data else None
            
            if challenger_price and self.should_swap(incumbent_score, challenger_score):
                # Sell incumbent
                pnl = (current_price - pos.entry_price) * pos.units
        ...
        for challenger in challengers:
            ...
            # Use fallback for candidate price (Open price)
            price_data = self.fetch_price_with_fallback(symbol, week_date)
            if not price_data:
                continue
            ...
            if sizing['shares'] <= 0:
                print(f"    Skipping {symbol}: Zero shares (sizing)")
                continue
            ...
            if cost > self.current_capital:
                ...
                if affordable_shares <= 0:
                    print(f"    Skipping {symbol}: Insufficient capital (need ₹{cost:,.0f}, have ₹{self.current_capital:,.0f})")
                    continue
            ...
            vacancies -= 1
            print(f"    BOUGHT {symbol} - Remaining Vacancies: {vacancies}")
    ...
    def run(self) -> List[BacktestResult]:
        ...
        print("=" * 60)
        print("Starting Weekly Backtest (CSV Rankings & Open Prices)")
        ...
        for i, monday in enumerate(mondays, 1):
            ...
            # Fetch rankings from CSV (Top 200 pool)
            raw_candidates = self.fetch_rankings(monday)
            
            if not raw_candidates:
                print(f"  No rankings available in CSV for {monday}, skipping...")
                continue
            
            # Apply Filters: Price and Market Cap
            top_rankings = []
            for candidate in raw_candidates:
                symbol = candidate['tradingsymbol']
                
                # 1. Price Filter (Get current price)
                price_data = self.fetch_price_with_fallback(symbol, monday)
                if not price_data:
                    continue
                ...
                # 2. Market Cap Filter
                shares = self.shares_map.get(symbol)
                if shares:
                    mcap_crore = (shares * price) / 10_000_000
                    if mcap_crore < MIN_MARKET_CAP_CRORE:
                        continue
                elif self.shares_map:
                    # If shares data exists but not for this stock, skip it? 
                    # User said: "Do not invest in stocks where market cap is less than 500 crore"
                    # This implies we need the data to invest.
                    continue
                
                # Store the fetched price to avoid redundant API calls later
                candidate['price'] = price
                top_rankings.append(candidate)
                
                if len(top_rankings) >= TOP_N:
                    break
            
            if not top_rankings:
                print(f"  No candidates met the filter criteria for {monday}")
                continue

            top_symbols = [r['tradingsymbol'] for r in top_rankings]
            print(f"  {len(top_rankings)} candidates passed filters (from pool of {len(raw_candidates)})")
            
            # Build lookups for ALL required stocks (Top Filtered + currently held)
            score_lookup = {r['tradingsymbol']: r['composite_score'] for r in top_rankings}
            price_lookup = {r['tradingsymbol']: r['price'] for r in top_rankings}
            
            # Fetch open prices with fallback for currently held positions
            for sym, pos in list(self.positions.items()):
                price_data = self.fetch_price_with_fallback(sym, monday)
                if price_data:
                    price_lookup[sym] = price_data['price']
                    # Also update score if available in CSV
                    ranking_data = self.fetch_ranking_by_symbol(sym, monday)
                    if ranking_data:
                        score_lookup[sym] = ranking_data.get('composite_score', pos.composite_score)
                        pos.composite_score = score_lookup[sym]
                else:
                    # If couldn't find price even with fallback, use last entry (warning)
                    print(f"    Warning: No price found for held {sym} on {monday} even with fallback")
                    price_lookup[sym] = pos.entry_price # Fallback
            
            # Rebalance portfolio
            actions = self.rebalance_portfolio(monday, top_rankings, score_lookup, price_lookup)
            
            # Calculate current portfolio value (using updated prices)
            ...
            for sym, pos in self.positions.items():
                ...
                invested_amount += pos.units * current_price
                total_risk += (current_price - pos.current_stop_loss) * pos.units
            ...
            result = BacktestResult(
                ...
                actions=actions,
                top_10_stocks=top_symbols[:MAX_POSITIONS],
