class Strategy1Parameters:
    ...
    # Main factor weights (total = 1.0)
    trend_strength_weight: float = 0.30
    momentum_velocity_weight: float = 0.30  # Updated from 0.25
    risk_efficiency_weight: float = 0.20
    conviction_weight: float = 0.15
    structure_weight: float = 0.05  # Updated from 0.10
...
class TransactionCostConfig:
    ...
    brokerage_percent: float = 0.0003  # 0.03%
    brokerage_cap: float = 20.0  # ₹20 max per order
    stt_percent: float = 0.001  # 0.1% sell only
    exchange_percent: float = 0.0000345  # ~0.00345%
    sebi_per_crore: float = 10.0  # ₹10 per crore
    stamp_duty_percent: float = 0.00015  # 0.015% buy only
    gst_percent: float = 0.18  # 18% on taxable
